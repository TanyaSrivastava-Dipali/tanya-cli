...
import { AccountID, Address, Engine, base58ToHex, formatU256, ethErc20ToNep141, } from 'tanya-evm';
...
async function main(argv, env) {
    program
        ...
        .option('--network <network>', 'specify LOX network ID', env.NEAR_ENV || 'local')
        .option('--endpoint <url>', 'specify LOX RPC endpoint URL', env.NEAR_URL)
        .option('--engine <account>', 'specify Lox-evm Engine account ID', env.AURORA_ENGINE)
    ...
    program
        ...
        const p = new Table();
        p.addRow({ contract: address, inputs: input, output: `0x${output ? Buffer.from(output).toString('hex') : ''}` });
        p.printTable();
    });
    program
    ...
    program
        ...
        const p = new Table();
        p.addRow({ contract: address, inputs: input, output: `0x${output ? output.toString() : ''}` });
        p.printTable();
    });
    program
        ...
        const p = new Table();
        p.addRow({ contract: address, code: `0x${code ? Buffer.from(code).toString('hex') : ''}` });
        p.printTable();
    });
    program
    ...
    program //
        ...
        if (config.debug) {
            ...
            for (const record of result.values()) {
                ...
                const p = new Table();
                p.addRow({ address: address, nonce: nonce, balance: balance, code: code ? code.length : 0 });
                p.printTable();
                const p2 = new Table();
    ...
    program
        ...
        const p = new Table();
        p.addRow({ account: account, encoded: AccountID.parse(account).unwrap().toAddress().toString() });
        p.printTable();
    });
    program
    ...
    program
        .command('get-lox_evm-erc20 <tokenAddress>')
        .action(async (tokenAddress, options, command) => {
        const erc20Ethereum = Address.parse(tokenAddress).unwrap();
        const [config, engine] = await loadConfig(command, options, env);
        const nep141Lox = ethErc20ToNep141(erc20Ethereum, config.network);
        const lox_evmAddress = (await engine.getLox_evmErc20Address(nep141Lox)).unwrap();
        console.log(lox_evmAddress.toString());
    });
    program.parse(argv);
}
